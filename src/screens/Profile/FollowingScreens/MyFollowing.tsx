...
import { useFollowUserMutation, useGetAllMyFollowingMutation, useUnfollowUserMutation } from '@social/redux/services/auth/authApi';
...
if (isAllFollowingLoading && page === 1 && followings?.length===0 ) {
  ...
  return (
    <View style={styles.myFollowersContainer}>
      ...
      <View>
      {followings.length === 0 ? (
                    <EmptyMessage header="Add Following" description="Once you follow people, you'll see them here" />
                ) : (
        <FlatList
          data={followings}
